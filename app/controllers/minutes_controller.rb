class MinutesController < ApplicationController
  ...
  def index
    ...
    if params['date'] != nil
      @minutes_date = @minutes.where("created_at = ?", params['date'].to_date.in_time_zone.strftime("%Y-%m-%d") + '%')
    else
      @minutes_date = @minutes.where("created_at = ?", Date.current.in_time_zone.strftime("%Y-%m-%d") + '%')
    end
    @category_total = @minutes_date.group(:category).sum(:total)
    if params['date'] != nil
      @month = @minutes.where("created_at = ?", params['date'].to_date.in_time_zone.strftime("%Y-%m") + '%')
    else
      @month = @minutes.where("created_at = ?", Date.current.in_time_zone.strftime("%Y-%m") + '%')
    end
